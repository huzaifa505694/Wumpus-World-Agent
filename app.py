...
SERVER_STATES = {}
# HUNTBOT — Propositional Logic Agent
# SCORING CRITERIA:
#   +1000  →  Gold collected
#   -1     →  Each move (step penalty)
#   -10    →  Arrow fired
#   (game over on death — no extra penalty)
# ─────────────────────────────────────────────────────────────────
...
def do_step(state):
    ...
    if state["wumpus_alive"] and agent["has_arrow"] and len(kb["wumpus_confirmed"]) > 0:
        ...
        if (wr, wc) in neighbors(r, c, state["rows"], state["cols"]):
            ...
            state["log"].append({"msg": f"🏹 Arrow fired! BEAST at ({wr},{wc}) ELIMINATED! -10", "type": "warn"})
